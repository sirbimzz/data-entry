...
for i, row in df_Tables.iterrows():
    ...
    if df_Tables.at[i, 'reportFreq']=="Daily" and df_Tables.at[i, 'getPrompt']=="Yes" and lastUpdate.strftime('%d/%m/%Y') != datetime.today().strftime('%d/%m/%Y'):
        ...
        count = 0
        new_date = start_date
        for b in range(diff_days):
            if df_data.shape[0] == 0:
                record_found = "NO"
                count = 1
            else:
                for b, row in df_data.iterrows():
                    if df_data.at[b, 'RecordDate']==new_date.strftime('%d/%m/%Y'):
                        record_found = "YES"
                        break
                    else:
                        record_found = "NO"
                        count = count+1
            if record_found == "NO":
                blank_dates = blank_dates + new_date.strftime('%d-%b-%Y') + "\n"
        ...
        if count!=0:
            ...
            msg = MIMEText("""Dear User\n\nPlease note that the data for the underlisted dates are pending update(s) on the """ + curr_table + """ table.\n\n""" 
            ...
            #recipients = ['Abimbola.Salami@nlng.com', 'Abimbola.Salami@nlng.com']
    ...
    elif df_Tables.at[i, 'reportFreq']=="Weekly" and df_Tables.at[i, 'getPrompt']=="Yes" and ((week_no != week_no2 and lastUpdate.year == datetime.today().year) or (week_no == week_no2 and lastUpdate.year != datetime.today().year)) and day_no == day_no2:
        curr_table = df_Tables.at[i, 'tableName']
        ...
        for b in range(diff_wks):
            if df_data.shape[0] == 0:
                ...
                for b, row in df_data.iterrows():
                    record_date = datetime.strptime(df_data.at[b, 'RecordDate'], '%d/%m/%Y')
        ...
        if count!=0:
            ...
            msg = MIMEText("""Dear User\n\nPlease note that the data for the underlisted week(s) are pending update(s) on the """ + curr_table + """ table.\n\n""" 
                    + blank_dates + """\nKindly follow the link below to provide required updates.\n\n""" + """http://bny-s-560/datacentric/dataEntry/datatable.html?""" + curr_table + """\n\nRegards\nData Entry Portal""")
            sender = 'noreply-dataentry@nlng.com'
            msg['Subject'] = "Data Pending Update on " + curr_table
            msg['From'] = sender
            msg['To'] = to
    ...
    elif df_Tables.at[i, 'reportFreq']=="Monthly" and df_Tables.at[i, 'getPrompt']=="Yes" and ((week_no != week_no2 and lastUpdate.year == datetime.today().year) or (week_no == week_no2 and lastUpdate.year != datetime.today().year)) and day_no == 0:
        curr_table = df_Tables.at[i, 'tableName']
        ...
        start_date = datetime.strptime(df_Tables.at[i, 'dateCreated'], '%d/%m/%Y')
        this_date = datetime.today()
        diff_months = (relativedelta(this_date, start_date)).months
        blank_dates = ""
        count = 0
        new_date = start_date
        if this_date.day > int(df_Tables.at[i, 'reportDate']):
            rng = diff_months+1
        else:
            rng = diff_months
            ...
            if df_data.shape[0] == 0:
                ...
                for b, row in df_data.iterrows():
                    ...
                    if record_date.month==new_date.month and record_date.year==new_date.year:
                        ...
                        count = count+1
            if record_found == "NO":
                blank_dates = blank_dates + get_mth_nme(new_date.month) + ", " + str(new_date.year) + "\n"
        ...
        if count!=0:
            to=to[:-2]
            s = smtplib.SMTP('webmail.nlng.com', 25)
            msg = MIMEText("""Dear User\n\nPlease note that the data for the underlisted month(s) are pending update(s) on the """ + curr_table + """ table.\n\n""" 
                    + blank_dates + """\nKindly follow the link below to provide required updates.\n\n""" + """http://wapp-bny.nlng.net/datacentric/dataEntry/datatable.html?""" + curr_table + """\n\nRegards\nData Entry Portal""")
            sender = 'noreply-dataentry@nlng.com'
            msg['Subject'] = "Data Pending Update on " + curr_table
            msg['From'] = sender
    ...
    elif df_Tables.at[i, 'reportFreq']=="Annually" and df_Tables.at[i, 'getPrompt']=="Yes" and ((lastUpdate.month != datetime.today().month and lastUpdate.year == datetime.today().year) or (lastUpdate.month == datetime.today().month and lastUpdate.year != datetime.today().year)) and datetime.today().day == 1:
        curr_table = df_Tables.at[i, 'tableName']
        ...
        start_date = datetime.strptime(df_Tables.at[i, 'dateCreated'], '%d/%m/%Y')
        this_date = datetime.today()
        diff_years = this_date.year - start_date.year
        blank_dates = ""
        count = 0
        new_date = start_date
        if this_date.month > int(get_mth_num(df_Tables.at[i, 'reportMonth'])):
            rng = diff_years+1
        else:
            rng = diff_years
        ...
        for b in range(rng):
            if df_data.shape[0] == 0:
                ...
                for b, row in df_data.iterrows():
                    record_date = datetime.strptime(df_data.at[b, 'RecordDate'], '%d/%m/%Y')
                    if record_date.year==new_date.year:
                        record_found = "YES"
                        break
                    else:
                        record_found = "NO"
                        count = count+1
            if record_found == "NO":
                blank_dates = blank_dates + str(new_date.year) + "\n"
        ...
        if count!=0:
            to=to[:-2]
            s = smtplib.SMTP('webmail.nlng.com', 25)
            msg = MIMEText("""Dear User\n\nPlease note that the data for the underlisted year(s) are pending update(s) on the """ + curr_table + """ table.\n\n""" 
                    + blank_dates + """\nKindly follow the link below to provide required updates.\n\n""" + """http://wapp-bny.nlng.net/datacentric/dataEntry/datatable.html?""" + curr_table + """\n\nRegards\nData Entry Portal""")
            sender = 'noreply-dataentry@nlng.com'
            msg['Subject'] = "Data Pending Update on " + curr_table
            msg['From'] = sender
